...
export default function Chat({onBack, myData, selectedUser}) {
  ...
  return (
    <>
      <Pressable  style={[styles.actionBar,{marginTop: 10} ]}>
        <TouchableOpacity onPress={onBack} style={{flex:1}}>
          <Icon  name='arrow-left' size={20} style={{padding:10}} />
        </TouchableOpacity>
        <Text style={{ fontWeight: "bold", fontSize: 20}}>{selectedUser?selectedUser.username:""}</Text>
      </Pressable>
      <GiftedChat
        messages={messages}
